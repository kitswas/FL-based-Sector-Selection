...
class ModelHandler:
    ...
    def createArchitecture(
        ...
        if model_type == "coord_mlp":
            ...
            layer = Conv1D(
                20, 2, padding="SAME", activation="relu", name="coord_conv2"
            )(layer)
            ...
            layer = Conv1D(
                20, 2, padding="SAME", activation="relu", name="coord_conv4"
            )(layer)
            ...
            if fusion:
                out = Dense(256, activation="tanh", name="image_output")(layer)
                # out = Lambda(custom_function)(layer)
                architecture = Model(inputs=input_img, outputs=out)
            else:
                if strategy == "one_hot":
                    out = Dense(num_classes, activation="softmax", name="image_output")(
                        layer
                    )
                elif strategy == "reg":
                    out = Dense(num_classes)(layer)
            ...
            if fusion:
                ...
                out = Dropout(0.2, name="lidar_output")(
                    layer
                )  # 0.25 is similar ... could try more values
