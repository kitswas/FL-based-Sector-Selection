...
from keras.optimizers import Adadelta, SGD, Nadam, Adamax, Adagrad
...
RF_train, RF_val, RF_test, RF_c1, RF_c2, RF_c3, RF_c4 = get_data(
    selected_paths, "rf", "rf", args.test_all, args.test_all_path
)
...
if "coord" in args.input:
    ...
    X_coord_train, X_coord_validation, X_coord_test, gps_c1, gps_c2, gps_c3, gps_c4 = (
        get_data(selected_paths, "gps", "gps", args.test_all, args.test_all_path)
    )
    print(
        "GPS data shapes",
        X_coord_train.shape,
        X_coord_validation.shape,
        X_coord_test.shape,
    )
    ...
    print(
        "max over dataset b",
        np.max(abs(X_coord_train)),
        np.max(abs(X_coord_validation)),
        np.max(abs(X_coord_test)),
    )
...
if "img" in args.input:
    ...
    X_img_train, X_img_validation, X_img_test, img_c1, img_c2, img_c3, img_c4 = (
        get_data(selected_paths, "image", "img", args.test_all, args.test_all_path)
    )
    print(
        "max over dataset b",
        np.max(X_img_train),
        np.max(X_img_validation),
        np.max(X_img_test),
    )
    print(
        "image data shapes", X_img_train.shape, X_img_validation.shape, X_img_test.shape
    )
    ###normalize images
...
if "lidar" in args.input:
    ...
    X_lidar_train, X_lidar_validation, X_lidar_test, lid_c1, lid_c2, lid_c3, lid_c4 = (
        get_data(selected_paths, "lidar", "lidar", args.test_all, args.test_all_path)
    )
    print(
        "lidar data shapes",
        X_lidar_train.shape,
